...
std::unordered_map<int, Rule> RadialLayoutBuilder::groupRulesById(const std::vector<Rule> &rules) {
...
std::unordered_map<int, std::vector<SingleMetric> > RadialLayoutBuilder::groupRulesBySingleMetrics(const std::vector<Rule> &rules) {
...
std::unordered_map<int, std::vector<int> > RadialLayoutBuilder::groupRulesByConsequent(const std::vector<Rule> &rules) {
...
std::vector<int> RadialLayoutBuilder::sortByMetric(
...
std::unordered_map<int, std::vector<RadialLayoutBuilder::Path> >
RadialLayoutBuilder::groupPathsByConsequent(
    std::unordered_map<int, Rule> &rules_by_id,
...
std::map<RulePath, RadialLayoutBuilder::Path> RadialLayoutBuilder::groupMetricsByPathID(std::vector<Path> const &paths) {
...
std::map<RulePath, std::vector<RulePath> > RadialLayoutBuilder::calculateChildren(const std::map<RulePath, Path> &metrics_by_path_id) {
...
std::pair<RulePath, std::map<RulePath, int> > RadialLayoutBuilder::calculateLeafCounts(
...
std::pair<std::map<RulePath, double>, std::map<RulePath, double> > RadialLayoutBuilder::getSupportAndLiftNodes(const std::map<RulePath, Path> &metrics_by_path_id) {
...
int RadialLayoutBuilder::determineMaxDepth(const std::map<RulePath, Path> &metrics_by_path_id) {
    int max_depth = 1;
...
std::pair<std::map<RulePath, double>, std::map<RulePath, double> > RadialLayoutBuilder::computeAngularSpans(
    const RulePath &root,
    const int max_depth,
    std::map<RulePath, std::vector<RulePath> > children,
...
std::map<RulePath, std::tuple<double, double, double> > RadialLayoutBuilder::buildNodes(
    double x_off,
    double z_off,
...
void RadialLayoutBuilder::buildEdges(const std::vector<Path> &paths, std::map<RulePath, std::tuple<double, double, double> > coordinates, std::vector<Edge> &all_edges) {
