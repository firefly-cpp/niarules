...
TOP_K_RINGS   <- 8      # e.g., keep top 8 most common ring values
...
while (length(below_quota()) && batch < MAX_BATCHES) {
  ...
  if (nrow(cand)) {
    ...
    if (nrow(cand)) {
      cand$key <- mapply(function(a, c, v) rule_key(a, c, v), cand$Antecedent, cand$Consequence, cand$v, USE.NAMES = FALSE)
      ...
      if (nrow(new_rules)) {
        ...
        for (vk in names(by_v)) {
          kept[[vk]] <- bind_rows(kept[[vk]], by_v[[vk]])
        }
      }
    }
  }
